...
class CPU_Builder:
    ...
    def build( self, prefix, nodeId, cpuId ):
        ...
        #cpuDcacheIf.addParam("debug", 1)
        #cpuDcacheIf.addParam("debug_level", 10)
        # CPU.mem interface for I-cache
        cpuIcacheIf = cpu.setSubComponent( "mem_interface_inst", "memHierarchy.standardInterface" )
        #cpuIcacheIf.addParam("debug", 1)
        #cpuIcacheIf.addParam("debug_level", 10)
        ...
        if env_tlb_config:
            dtlb = cpuDcacheIf.setSubComponent("tlb", "mmu." + tlbType)
            dtlb.addParams(tlbParams)

            itlb = cpuIcacheIf.setSubComponent("tlb", "mmu." + tlbType)
            itlb.addParams(tlbParams)
            cpuIcacheIf.addParam("tlb_exe_permission",True)

            link_cpu_l1dcache_link = sst.Link(prefix+".link_cpu_l1dcache_link")
            link_cpu_l1dcache_link.connect( (cpuDcacheIf, "lowlink", "1ns"), (cpu_l1dcache, "highlink", "1ns") )

            link_cpu_l1icache_link = sst.Link(prefix+".link_cpu_l1icache_link")
            link_cpu_l1icache_link.connect( (cpuIcacheIf, "lowlink", "1ns"), (cpu_l1icache, "highlink", "1ns") )
        else:
            # CPU data TLB
            ...
            link_cpu_dtlb_link.connect( (cpuDcacheIf, "lowlink", "1ns"), (dtlbWrapper, "highlink", "1ns") )
...
#link_os_ostlb_link.connect( (node_os_mem_if, "lowlink", "1ns"), (ostlbWrapper, "highlink", "1ns") )
...
#link_os_cache_link.connect( (ostlbWrapper, "lowlink", "1ns"), (os_cache, "highlink", "1ns") )
